...
```{r linearity check w/ Price > 400 Outliers Removed}
...
```{r linearity check w/ Log-Linear model Price > 400 Outliers Removed}
...
```{r multicollinearity assumption 2}
vif(log.depend.model)[,3]^2
```
...
```{r full model check for linearity}
...
- We are seeing it as a bad option because
  - The true relationship between the response and predictors is NOT “linear”.  The relationships are complex.
    - We have gotten close, but we have worked extremely hard in specifying our model and manipulate the raw data to surface a linear relationship
    - This makes the interpretation into the real world difficult to translate and understand
...
- Since we see a strong relationship between the categorical variables and we have not yet learned Random Forest models, we move forward with a Two-Way ANOVA model to create a sound model to predict the price of a NYC AirBnB.