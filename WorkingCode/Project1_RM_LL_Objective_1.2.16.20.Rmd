...
## Begin Ridge, LASSO, Elastic Net Analysis

```{r }

## Ridge, LASSO and Elastic Net Regression and Regularization
## Source:
...
```
## Log-Linear Elastic Net Regression, selecting alpha=.1

```{r Log-Elastic Net}
# Performing Log-Linear Elastic Net regression

cv.r <- cv.glmnet(xlog, ylog, alpha = .1) # alpha=.1 is chosen value for Elastic Net Regularization, choose minimum lambda
cv.r$lambda.min
model.ENL <- glmnet(xlog, ylog, alpha = .1, lambda = cv.r$lambda.min)
coef(model.ENL)
plot(cv.r)
